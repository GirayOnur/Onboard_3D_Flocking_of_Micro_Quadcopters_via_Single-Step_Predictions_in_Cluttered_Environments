%plots the minimum obstacle distance metric:
function [done,arrOd]=analyze_obstacle_distance(dt,posData,obsPosList,obsRadList,simStep,agentNum,obsNum,quadRad,dSafetyObs,obsRad)
    ...
    for i=1:simStep
        ...
        for n=1:agentNum
            quadPos = posArr(n,:);
            obsAgentDistArr = vecnorm(abs(obsPosList(:,1:2) - quadPos(:,1:2)),2,2);
    ...
    plot(arrTime,arrOd,'Color', [0.4940 0.1840 0.5560] , 'LineWidth', 1.2)
    hold on
