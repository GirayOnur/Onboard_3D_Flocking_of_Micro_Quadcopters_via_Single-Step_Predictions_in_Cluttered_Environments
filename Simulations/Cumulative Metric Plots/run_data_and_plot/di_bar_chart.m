...
meanList = [];
errList = [];
meanPos = [0.03,0.05,0.07];
for i=1:3
    errList(end+1) = calcStdErr(dataErrPlot1{i});
end
plot(meanPos,errList,'-s')
hold on


meanList = [];
errList = [];
meanPos = [0.03,0.05,0.07]+0.001;
for i=1:3
    meanList(end+1) = mean(dataErrPlot2{i});
    errList(end+1) = calcStdErr(dataErrPlot2{i});
end
plot(meanPos,errList,'-s')
...
meanList = [];
errList = [];
meanPos = [0.03,0.05,0.07]+0.002;
for i=1:3
    meanList(end+1) = mean(dataErrPlot3{i});
    errList(end+1) = calcStdErr(dataErrPlot3{i});
end
plot(meanPos,errList,'-s')
%}
...
ylabel('\boldmath${d_p}$','FontSize',30,'Interpreter','latex' )
