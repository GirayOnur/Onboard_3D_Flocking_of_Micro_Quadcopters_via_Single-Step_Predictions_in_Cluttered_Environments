...
function [done,arrOrder,arrAccuracy]=analyze_order_vel(dt,velData,simStep,agentNum,migVel,plotAccuracy,real)
    ign_kk = int16(0.5/dt);
    ...
    if plotAccuracy
        ...
        if real
            plot(arrTime,arrOrder, 'Color',[0.4940 0.1840 0.5560], 'LineWidth', 1.2) %normal 1.2, bold 3 (for zoom)
        else
            h2a = plot(arrTime,arrOrder, 'Color',[0 0.4470 0.7410], 'LineWidth', 1.2); %normal 1.2, bold 3 (for zoom)
            h2a.Color(4)=0.5; 
        end
        ylim([0 1])
