...
function [done]=analyze_normalized_speed_plot(dt,velData,simStep,agentNum,vSpd,real) %(dt,sim_vel_list,migVel,double(sim_kk),quadNum)
...
%     arrMin = lowpass(arrMin,1,20);
    if real
        ...
        hold on
        h2a = plot(arrTime,arrEv, 'Color', [0 0.4470 0.7410], 'LineWidth', 1);
        h2a.Color(4)=0.5;       
    end
